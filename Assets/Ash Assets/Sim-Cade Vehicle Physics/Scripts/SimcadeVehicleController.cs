...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        [Header("Other Things")] private RaycastHit[] wheelHits = new RaycastHit[4];
        ...
        void brakeLogic(float brakeInput)
        {
            ...
            float deltaSpeed = brakeAcceleration * brakeInput * Time.fixedDeltaTime *
        ...
        void AddSuspensionForce(Vector3 hardPoint, Transform wheel, float MaxSpringDistance, out RaycastHit wheelHit,
            out bool WheelIsGrounded, out float SuspensionForce)
        {
        ...
        void AddSuspensionForce_2(Vector3 hardPoint, Transform wheel, float MaxSpringDistance, out RaycastHit wheelHit,
            out bool WheelIsGrounded, out float SuspensionForce, int WheelNum)
        {
            var direction = -transform.up;
            ...
            if (WheelIsGrounded)
            {
                ...
                float offset = (MaxSpringDistance + 0.1f - wheelHit.distance) /
                               (MaxSpringDistance - wheelRadius - 0.1f);
                ...
                float totalDampingForce = Mathf.Clamp(-(vel * springDamper),
            ...
            float factor, float suspensionForce, int wheelNum)
        {
            if (wheelIsGrounded)
            {
                ...
                Vector3 sideVelocity =
                    (wheel.InverseTransformDirection(rb.GetPointVelocity(hardPoint)).x) * wheel.right;
                Vector3 forwardVelocity =
                    (wheel.InverseTransformDirection(rb.GetPointVelocity(hardPoint)).z) * wheel.forward;
                ...
                Vector3 contactDesiredAccel =
                    -Vector3.ProjectOnPlane(sideVelocity, SurfaceNormal) / Time.fixedDeltaTime;

                Vector3 frictionForce = Vector3.ClampMagnitude(
                ...
                float clampedFrictionForce = Mathf.Min(rb.mass / 4 * contactDesiredAccel.magnitude,
                    -Physics.gravity.y * rb.mass);
        ...
        void AckermannSteering(float steerInput)
        {
            ...
            {
                ackermennLeftAngle +=
                    Vector3.SignedAngle(transform.forward, rb.velocity + transform.forward, transform.up);
        ...
        void tireVisual(bool WheelIsGrounded, Transform wheel, Transform hardPoint, float hitDistance, int tireNum)
        {
            if (WheelIsGrounded)
            {
                ...
                {
                    wheel.localPosition = hardPoint.localPosition + (Vector3.up * wheelRadius) -
                                          Vector3.up * (hitDistance);
                }
                else
                {
                    wheel.localPosition =
            ...
            float minRotation = (Vector3.Dot(wheelVelocity, wheel.forward) / wheelRadius) * Time.fixedDeltaTime *
                                Mathf.Rad2Deg;
            float maxRotation = (Mathf.Sign(Vector3.Dot(wheelVelocity, wheel.forward)) * MaxSpeed / wheelRadius) *
